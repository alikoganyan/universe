...
import {
  View, Text, TouchableOpacity, Dimensions, Platform, TextInput
} from 'react-native';
...
const Input = (props) => {
  const {
    children, password = false, value, style, editable, inputStyle, labelStyle, keyboardType
  } = props;
  return (
    <FloatingLabel
      ...
      <Wrapper>
        ...
        <PhoneNumber>
          ...
          <StyledInput
            ...
            maxLength={10}
            style={{
              margin: 0, width: '75%', flex: 1, textAlign: 'left', paddingLeft: 20, color: error ? pink : black, borderColor: error ? pink : lightGrey1
            }}
            keyboardType="phone-pad"
          />
        </PhoneNumber>
    ...
    getRestorePassword = (e) => {
      ...
      (phone_number && phone && phone.length >= 9) && sendRequest({
        ...
        failFunc: (err) => {
          ...
          this.setState({
            error: <TouchableOpacity onPress={this.signup}>
              <ErrorText>
