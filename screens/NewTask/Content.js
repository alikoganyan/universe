...
const ReceiverComponent = (props) => {
  ...
  const {
    image, phone_number, role, first_name, last_name
  } = children;
  return (
    <Receiver last={last}>
      ...
      <Wrapper>
        ...
        >
          ...
          <StyledInput
            ...
            multiline
            style={{
              flex: 1, marginBottom: 30, textAlign: 'left', padding: 0
            }}
          />
          <DeadLine>
            <DialogsLabel>
...
Создать задачу
    ...
    proceed = (e) => {
      const {
        receivers, forward, setTasks, user, tasks
      } = this.props;
      const {
      ...
      sendRequest({
        ...
        attr: {
          task: {
            name: taskName, description: taskText, deadline, performers: [receivers[0]._id]
          }
        },
        success: (res) => {
          console.log(res);
