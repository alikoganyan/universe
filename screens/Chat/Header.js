...
class HeaderComponent extends Component {
    render() {
        ...
        const minutes = lastVisit.getMinutes() >= 10 ? lastVisit.getMinutes() : `0${lastVisit.getMinutes()}`;
        const hours = lastVisit.getHours() >= 10 ? lastVisit.getHours() : `0${lastVisit.getHours()}`;
        const date = lastVisit.getDate() >= 10 ? lastVisit.getDate() : `0${lastVisit.getDate()}`;
        ...
        const lastSeenDate = last_visit ?
            ...
            'был в течении часа' :
            lastVisitDifference < 1440 ? `был онлайн в ${hours}:${minutes}` :
            `был онлайн ${date} ${month}` : 'неизвестно';
        return (
            <Header>
		<Top>
		  <Left>
			  ...
			  <>
				...
				<ToProfile onPress={this.toProfile}>
				  ...
				  <Info>
					...
					<InfoParticipants numberOfLines={1}>{lastSeenDate}</InfoParticipants>
				  </Info>
				</ToProfile>
			  </>
			) : (
