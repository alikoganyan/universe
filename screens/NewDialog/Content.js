			...
			success: (res) => {
				...
				res.users.map((user) => {
					...
					const department = users.department
						.filter(e => e.title.name === user.department.name || e.title.name === 'без департамента')[0];
					if (department) {
						const index = users.department
							.findIndex(e => e.title.name === user.department.name || e.title.name === 'без департамента');
