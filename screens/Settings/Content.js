...
import { View, Text, SafeAreaView, FlatList, TouchableOpacity, Dimensions, ActivityIndicator, ScrollView } from 'react-native';
...
class Content extends Component {
  render() {
    ...
    return (
      <SafeAreaView>
        <Wrapper>
          <ScrollView style={{ height: Dimensions.get('window').height - HeaderHeight - 20 }}>
            <Box first>
  ...
  handleToggle = (e) => {
    ...
    item.option.value = !newSettings.filter(({ label }) => e === label)[0].option.value;
    this.setState({ settings: newSettings });
  }
